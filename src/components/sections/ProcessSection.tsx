...
const ProcessSection = () => (
  <section id="process" className="section-padding border-b-2 border-foreground bg-foreground text-background">
    <div className="container-custom">
      <div className="mb-9 flex flex-col justify-between gap-5 sm:mb-12 sm:gap-6 md:flex-row md:items-end">
        <div>
          <span className="eyebrow mb-6 border-background">From brief to coffee run</span>
          <h2 className="font-display text-4xl uppercase leading-[0.95] tracking-[-0.04em] sm:text-7xl sm:leading-[0.9] md:text-8xl">
            How it moves.
          </h2>
        </div>
      ...
      <div className="grid border-2 border-background md:grid-cols-3">
        {steps.map(([number, title, body], index) => (
          <article
            key={number}
            className={`relative p-6 sm:min-h-[300px] sm:p-9 ${index ? "border-t-2 border-background md:border-l-2 md:border-t-0" : ""}`}
          >
            <span className="font-display text-5xl leading-none text-accent sm:text-7xl">{number}</span>
            <h3 className="mt-6 font-display text-2xl uppercase leading-none sm:mt-12 sm:text-3xl">{title}</h3>
